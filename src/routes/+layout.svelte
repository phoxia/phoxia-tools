<script lang="ts">
	...
	import { initConsent, isAccepted, hasAnalyticsConsent, hasAdvertisingConsent } from '$lib/consent/consent.svelte';
	...
	$effect(() => {
		...
		if (hasAdvertisingConsent() && !document.getElementById('phoxia-ads') && !document.getElementById('phoxia-scripts')) {
			const adsScript = document.createElement('script');
			adsScript.id = 'phoxia-ads';
			adsScript.src = 'https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-7052672921293029';
			adsScript.async = true;
			adsScript.crossOrigin = 'anonymous';
...
<svelte:head>
	<script type="application/ld+json">
		{"@context":"https://schema.org","@type":"WebSite","name":"Phoxia Tools","url":"https://tools.phoxia.org","description":"Free developer tools that run entirely in your browser.","potentialAction":{"@type":"SearchAction","target":"https://tools.phoxia.org/tools/http-status?q={search_term_string}","query-input":"required name=search_term_string"}}
	</script>
...
<div class="page-shell">
	...
	<div aria-live="polite" class="sr-only" style="position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;">{getAnnouncement()}</div>
	<Footer />
</div>
