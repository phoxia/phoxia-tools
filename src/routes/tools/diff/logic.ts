...
function backtrack(dp: number[][], a: string[], b: string[], i: number, j: number, out: DiffLine[], lLeft: number[], lRight: number[]): void {
	if (i === 0 && j === 0) return;
...
export function computeDiff(left: string, right: string): DiffLine[] {
	...
	if (a.length > MAX_DIFF_LINES || b.length > MAX_DIFF_LINES) {
		return [{ type: 'removed', text: `Diff too large (max ${MAX_DIFF_LINES} lines per side)`, leftLine: 1 }];
	}
