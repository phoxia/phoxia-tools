...
import { copyToClipboard } from '$lib/clipboard.svelte';
	...
	const groups = $derived.by(() => {
		...
		const groupDefs: { label: string; color: string; prefix: number }[] = [
			{ label: t().tools.httpStatus.groups.informational, color: 'var(--color-text-muted)', prefix: 1 },
	...
	const filtered = $derived.by(() => {
		...
		return groups.map((g) => ({
			...
			codes: g.codes.filter(
				(c) => String(c.code).includes(q) || c.name.toLowerCase().includes(q) || c.desc.toLowerCase().includes(q)
			),
		})).filter((g) => g.codes.length > 0);
	});
...
<ToolLayout toolId="httpStatus" route="/tools/http-status" title={t().tools.httpStatus.name} description={t().tools.httpStatus.desc}>
	<div style="display: flex; flex-direction: column; gap: 1.5rem;">
		...
		{#each filtered as group}
			<div>
				<h3 style="font-size: 0.75rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: {group.color}; margin: 0 0 0.625rem;">
					{group.label}
				</h3>
				<div style="display: flex; flex-direction: column; gap: 1px; border-radius: var(--radius); overflow: hidden; border: 1px solid var(--color-border);">
					{#each group.codes as entry}
						<div style="display: flex; align-items: baseline; gap: 1rem; padding: 0.6rem 0.875rem; background: var(--color-surface);">
							<button
							...
							<span style="flex-shrink: 0; min-width: 12rem; font-size: 0.85rem; font-weight: 600; color: var(--color-text);">
								{entry.name}
							</span>
		...
		{#if filtered.length === 0}
			<p style="color: var(--color-text-muted); font-size: 0.875rem; text-align: center; padding: 2rem 0;">
