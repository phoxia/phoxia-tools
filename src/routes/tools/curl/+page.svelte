...
<ToolLayout toolId="curl" title={t().tools.curl.name} description={t().tools.curl.desc}>
	<div style="display: flex; flex-direction: column; gap: 1rem;">
		<div>
			<label for="curl-input" style="display: block; font-size: 0.85rem; margin-bottom: 0.375rem; color: var(--color-text-muted);">
		...
		<div style="display: flex; gap: 0.5rem;">
			...
			<button onclick={() => { input = EXAMPLE; trackToolUsed('curl'); }}
		...
		{#if result}
			...
			{:else}
				...
				<div style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.8rem;">
					...
					">{result!.method}</span>
					<span style="font-family: var(--font-mono); color: var(--color-text-muted); word-break: break-all;">{result!.url}</span>
				</div>
				...
				<div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius); overflow: hidden;">
					...
					<pre style="margin: 0; padding: 1rem; font-family: var(--font-mono); font-size: 0.8rem; line-height: 1.6; overflow-x: auto; color: var(--color-text);">{tab === 'fetch' ? result!.fetch : result!.axios}</pre>
				</div>
			{/if}
		{/if}
	</div>
