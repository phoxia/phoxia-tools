...
function tokenize(cmd: string): string[] {
	...
	for (const ch of cmd.trim()) {
		if (quote) {
			if (ch === quote) { quote = ''; tokens.push(current); current = ''; }
			else current += ch;
		} else if (ch === "'" || ch === '"') {
			quote = ch;
		} else if (ch === ' ' || ch === '\t') {
...
export function parseCurl(cmd: string): CurlResult {
	...
	if (tokens[0]?.toLowerCase() !== 'curl') {
		return { method: 'GET', url: '', headers: {}, body: null, fetch: '', axios: '', error: t().tools.curl.errorNotCurl };
	}
	...
	if (!url) return { method: 'GET', url: '', headers: {}, body: null, fetch: '', axios: '', error: t().tools.curl.errorNoUrl };
...
function buildFetch(method: string, url: string, headers: Record<string, string>, body: string | null): string {
...
function buildAxios(method: string, url: string, headers: Record<string, string>, body: string | null): string {
