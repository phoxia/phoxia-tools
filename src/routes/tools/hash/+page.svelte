...
<ToolLayout toolId="hash" title={t().tools.hash.name} description={t().tools.hash.desc}>
	<div style="display: flex; flex-direction: column; gap: 1rem;">
		<div>
			<label for="hash-input" style="display: block; font-size: 0.85rem; margin-bottom: 0.375rem; color: var(--color-text-muted);">
			...
			<div style="display: flex; flex-direction: column; gap: 0.5rem;">
				{#each algos as algo}
					<div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius); overflow: hidden;">
						<div style="display: flex; justify-content: space-between; align-items: center; padding: 0.375rem 0.75rem; border-bottom: 1px solid var(--color-border);">
							<span style="font-size: 0.7rem; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: var(--color-text-faint);">{algo}</span>
							<button onclick={() => copy(results[algo], algo)}
						...
						<code style="display: block; padding: 0.625rem 0.75rem; font-family: var(--font-mono); font-size: 0.78rem; word-break: break-all; color: var(--color-text); line-height: 1.5;">
							{results[algo]}
						</code>
					</div>
				{/each}
