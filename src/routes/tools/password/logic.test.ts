...
describe('generatePassword', () => {
	...
	it('includes uppercase when requested', () => {
		const p = generatePassword({ length: 100, upper: true, lower: false, digits: false, symbols: false });
		expect(/[A-Z]/.test(p)).toBe(true);
		expect(/[a-z]/.test(p)).toBe(false);
	});

	it('includes digits when requested', () => {
		const p = generatePassword({ length: 100, upper: false, lower: false, digits: true, symbols: false });
		expect(/[0-9]/.test(p)).toBe(true);
	});

	it('includes symbols when requested', () => {
		const p = generatePassword({ length: 100, upper: false, lower: false, digits: false, symbols: true });
		expect(/[^a-zA-Z0-9]/.test(p)).toBe(true);
	});

	it('generates unique passwords', () => {
		const passwords = new Set(Array.from({ length: 20 }, () =>
