...
<ToolLayout toolId="jwt" title={t().tools.jwt.name} description={t().tools.jwt.desc}>
	<div style="display: flex; flex-direction: column; gap: 1rem;">
		<div>
			<label for="jwt-input" style="display: block; font-size: 0.85rem; margin-bottom: 0.375rem; color: var(--color-text-muted);">
			...
			<textarea
				...
				spellcheck={false}
				oninput={() => { if (input.trim()) decode(); else result = null; }}
				style="
		...
		{#if result}
			{#if result.error}
				<p role="alert" style="color: var(--color-error); font-size: 0.875rem; margin: 0; font-family: var(--font-mono);">{errorMessage}</p>
			{:else}
				<!-- exp banner -->
				{#if expStatus}
					<div style="
						padding: 0.5rem 0.875rem; border-radius: var(--radius); font-size: 0.8rem;
						background: {expStatus!.expired ? 'color-mix(in srgb, var(--color-error) 10%, transparent)' : 'color-mix(in srgb, var(--color-success) 10%, transparent)'};
						color: {expStatus!.expired ? 'var(--color-error)' : 'var(--color-success)'};
						border: 1px solid {expStatus!.expired ? 'color-mix(in srgb, var(--color-error) 30%, transparent)' : 'color-mix(in srgb, var(--color-success) 30%, transparent)'};
					">
						{expStatus!.expired ? '⚠ ' + t().tools.jwt.expired : '✓ ' + t().tools.jwt.valid} · exp: {expStatus!.date}
					</div>
				{/if}

				<!-- Three panels -->
				{#each [
					{ label: t().tools.jwt.header, data: result.header, raw: result.raw?.header, key: 'header' },
					{ label: t().tools.jwt.payload, data: result.payload, raw: result.raw?.payload, key: 'payload' },
					{ label: t().tools.jwt.signature, data: null, raw: result.signature, key: 'sig' }
				] as panel}
					<div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius); overflow: hidden;">
						...
						">
							<span style="font-size: 0.75rem; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: var(--color-text-faint);">
								{panel.label}
							</span>
							<button onclick={() => copy(panel.data ? JSON.stringify(panel.data, null, 2) : panel.raw!, panel.key)}
						...
						<pre style="margin: 0; padding: 0.875rem; font-family: var(--font-mono); font-size: 0.8rem; line-height: 1.6; overflow-x: auto; color: var(--color-text);">{panel.data ? JSON.stringify(panel.data, null, 2) : panel.raw}</pre>
					</div>
				{/each}
			{/if}
		{/if}
