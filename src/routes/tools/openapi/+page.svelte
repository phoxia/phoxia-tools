    ...
    try {
      ...
      if (!spec.openapi)
        return {
          ...
          servers: [],
          tags: {},
          error: t().tools.openapi.errorMissing,
        };
      ...
      return {
        ...
        servers: [],
        tags: {},
        error: t().tools.openapi.errorParse.replace("{msg}", (e as Error).message),
      };
    }
  }
  ...
  <div style="display: flex; flex-direction: column; gap: 1rem;">
    ...
    {#if parsed}
      ...
      {:else}
        <!-- Info card -->
        <div
          style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius); padding: 1rem 1.25rem;"
        >
          <div
            style="display: flex; align-items: baseline; gap: 0.75rem; margin-bottom: {parsed.description
              ? '0.5rem'
              : '0'};"
          >
            <h2 style="margin: 0; font-size: 1.1rem; color: var(--color-text);">{parsed.title}</h2>
            <span
              style="font-family: var(--font-mono); font-size: 0.75rem; color: var(--color-text-faint); background: color-mix(in srgb, var(--color-accent) 10%, transparent); padding: 0.1rem 0.5rem; border-radius: 99px;"
              >v{parsed.version}</span
            >
          </div>
        ...
        {#each Object.entries(parsed.tags) as [tag, endpoints]}
          ...
          >
            ...
            >
              <span
                style="font-weight: 700; font-size: 0.875rem; color: var(--color-text); text-transform: capitalize;"
                >{tag}</span
              >
              <span style="font-size: 0.75rem; color: var(--color-text-faint);"
                >{t().tools.openapi.endpointCount.replace("{n}", String(endpoints.length))}
                {tagOpen ? "▲" : "▼"}</span
              >
            </button>
            ...
            {#if tagOpen}
              <div style="border-top: 1px solid var(--color-border);">
                {#each endpoints as ep, i}
                  ...
                  <div
                    style="border-bottom: {i < endpoints.length - 1
                  ...
                  >
                    ...
                    >
                      ...
                      >
                      <code
                        style="font-family: var(--font-mono); font-size: 0.8rem; color: var(--color-text); flex: 1;"
                        >{ep.path}</code
                      >
                      <span
                        style="font-size: 0.78rem; color: var(--color-text-muted); white-space: nowrap;"
                        >{ep.summary !== ep.path ? ep.summary : ""}</span
                      >
                      <span style="color: var(--color-text-faint); font-size: 0.75rem;"
                        >{epOpen ? "▲" : "▼"}</span
                      >
                    </button>
                    ...
                    {#if epOpen}
                      <div
                        style="padding: 0.75rem 1rem 0.875rem; border-top: 1px solid color-mix(in srgb, var(--color-border) 50%, transparent); background: color-mix(in srgb, var(--color-surface) 60%, transparent);"
                      >
                        {#if ep.operationId}
                          <p
                            style="margin: 0 0 0.375rem; font-size: 0.78rem; color: var(--color-text-muted);"
                          >
                            <strong>{t().tools.openapi.operationId}:</strong>
                            <code style="font-family: var(--font-mono);">{ep.operationId}</code>
                          </p>
                        {/if}
                        {#if Object.keys(ep.responses).length > 0}
                          <p
                            style="margin: 0 0 0.25rem; font-size: 0.78rem; font-weight: 600; color: var(--color-text-muted);"
                          >
                          ...
                          <div style="display: flex; flex-direction: column; gap: 0.125rem;">
                            {#each Object.entries(ep.responses) as [code, resp]}
                              ...
                              >
                                <span
                                  style="color: {code.startsWith('2')
                                    ? 'var(--color-success)'
                                    : code.startsWith('4')
                                      ? 'var(--color-error)'
                                      : 'var(--color-text-muted)'}; font-weight: 700; min-width: 2.5rem;"
                                  >{code}</span
                                >
