...
<ToolLayout toolId="uuid" title={t().tools.uuid.name} description={t().tools.uuid.desc}>
	<div style="display: flex; flex-direction: column; gap: 1rem;">
		<div style="display: flex; align-items: center; gap: 1rem; flex-wrap: wrap;">
			<label style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.875rem; color: var(--color-text-muted);">
		...
		{#if uuids.length > 0}
			<div>
				<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.375rem;">
					<span style="font-size: 0.85rem; color: var(--color-text-muted);">{t().tools.uuid.generatedCount.replace('{n}', String(uuids.length))}</span>
					<button
						onclick={copy}
