  ...
  const result = $derived.by(() => {
    ...
    try {
      ...
      const re2 = new RegExp(
        pattern,
        ([...flags].includes("g") ? [...flags] : [...flags, "g"]).join("")
      );
      ...
      return {
        matches: [],
        highlighted: [{ text: testStr, match: false }],
...
<ToolLayout toolId="regex" title={t().tools.regex.name} description={t().tools.regex.desc}>
  <div style="display: flex; flex-direction: column; gap: 1rem;">
    ...
    <div>
      ...
      <div
        style="display: flex; align-items: center; gap: 0; background: var(--color-surface); border: 1px solid {result.error
          ? 'var(--color-error)'
          : 'var(--color-border)'}; border-radius: var(--radius); overflow: hidden;"
      >
        <span
          style="padding: 0 0.625rem; color: var(--color-text-faint); font-family: var(--font-mono); font-size: 1rem; user-select: none;"
          >/</span
        >
        <input
          id="regex-pattern"
      ...
      {#if !pattern}
        <p style="color: var(--color-text-muted); font-size: 0.8rem; margin: 0.25rem 0 0;">
          {t().tools.regex.hintEmptyPattern}
        </p>
      {/if}
      {#if result.error}
        <p
          role="alert"
          style="color: var(--color-error); font-size: 0.8rem; margin: 0.25rem 0 0; font-family: var(--font-mono);"
        >
          {result.error}
        </p>
      {/if}
      ...
      <div>
        <div style="display: flex; justify-content: space-between; margin-bottom: 0.375rem;">
          <span style="font-size: 0.85rem; color: var(--color-text-muted);"
            >{t().tools.regex.labelMatches}</span
          >
          <span
            style="font-size: 0.8rem; font-weight: 600; color: {result.matches.length > 0
              ? 'var(--color-success)'
        ...
        <div style="display: flex; flex-direction: column; gap: 0.25rem;">
          ...
          {#each result.matches as m, i (i)}
            {#if m.groups.length > 0}
              ...
              >
                {t().tools.regex.matchLabel.replace("{n}", String(i + 1))}: {m.groups
                  .map((g, j) => `$${j + 1}="${g ?? "undefined"}"`)
                  .join("  ")}
              </div>
            {/if}
          {/each}
